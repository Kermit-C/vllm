...
Usage:
    ...
    for k in m:
        dm = m[k]['avg_us'] - f[k]['avg_us']
        pct = dm / m[k]['avg_us'] * 100
        print(f'{k:<24s}  main={m[k][\"avg_us\"]:8.1f}us  feat={f[k][\"avg_us\"]:8.1f}us  Δ={dm:+7.1f}us  ({pct:+.1f}%)')
    "
...
def make_inputs(N, T_per_seq, dims, device="cuda", dtype=torch.bfloat16):
    ...
    if _USE_SSM_STATE_INDICES:
        ssm_state = torch.randn(max_blocks, HV, V, K, device=device, dtype=torch.float32, generator=gen)
        indices = torch.arange(1, N + 1, device=device, dtype=torch.int32)
    else:
        ssm_state = torch.randn(N, HV, V, K, device=device, dtype=torch.float32, generator=gen)
        indices = None

    return q, k, v, g, beta, cu_seqlens, ssm_state, indices, scale, max_blocks


def bench_scenario(name, N, T, dims):
    """Run one benchmark scenario, return (avg_us, min_us, median_us)."""
    q, k, v, g, beta, cu_seqlens, ssm_state, indices, scale, max_blocks = make_inputs(N, T, dims)
    ...
    for _ in range(WARMUP):
        ...
        if _USE_SSM_STATE_INDICES:
            kwargs['ssm_state_indices'] = indices
